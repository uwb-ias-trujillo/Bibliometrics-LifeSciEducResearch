...
write.csv(file = "tables/shortData.csv", data %>% select( PY, TI) %>% sample_n(200))
```
## Research Questions
...
authorprod <- authorProdOverTime(data, k = 10, graph = FALSE)$graph + 
  labs(title = "Author productivity from 2008 - 2022", x = NULL, y = NULL) + 
  theme_minimal() +
  scale_x_discrete(labels = str_replace(
      str_to_title(data$Author),
      "[a-zA-Z]+$",
      str_to_upper(
        str_extract(
          data$Author,"[a-zA-Z]+$")
