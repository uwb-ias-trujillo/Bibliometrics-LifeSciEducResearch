...
authordata <- inner_join(as_tibble(results$Authors),indices$H, by = c("AU" = "Element")) %>%
  filter(n>5)
...
ggplot(authordata, aes(size = NP, x = n, y = h_index)) + 
  geom_jitter(aes(alpha = 0.1)) +
  geom_text(aes(label = AU), check_overlap = TRUE) #+
...
msistates <- msiuniv %>%
  ...
  #summarize(states,affiliations = affiliations/sum(affiliations))
view(msistates)
