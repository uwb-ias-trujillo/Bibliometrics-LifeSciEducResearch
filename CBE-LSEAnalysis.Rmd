...
# write.csv(file = "tables/shortData.csv", data %>% select( PY, TI) %>% dplyr::sample_n(200))
```
## Research Questions
...
AU2 <- df2 %>% group_by(Author) %>% summarise(medianY = median(year)) %>% full_join(AU, by = c("Author" = "AU")) %>% mutate(Author = fct_reorder(AU$Author,AU$medianY, .desc = TRUE))
