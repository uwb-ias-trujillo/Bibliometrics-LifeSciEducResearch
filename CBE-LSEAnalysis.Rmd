...
temp %>%
  ...
  arrange(perc) %>% 
  select(name) %>% 
  mutate(name = as.factor(name)) -> x_levels
  
temp %>% 
  filter(eligibility != "Ineligible" & eligibility != "Undetermined") %>%
  #mutate(name = factor(name, levels = x_levels)) %>% 
  ggplot(aes(x = factor(name, levels = x_levels), y = perc, fill = eligibility)) +
