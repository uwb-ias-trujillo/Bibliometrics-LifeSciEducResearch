...
spatial_data_World <- left_join(as.data.frame(results$Countries),
                          get_urbn_map(map = "countries", sf = TRUE),
                          by = c("Tab", "Countries"))

#Affiliation Map - World
world <- map_data("world")
world

affiliationMapWorld <- ggplot() +
  geom_map(data = results$Countries, 
           map  = world,
           mapping = aes(map_id = region, fill = Freq),
           color = "#ffffff", size = 0.25) +
  labs(fill = "Author affiliations",
       title = "Number of institutions in author affilitations by country",
    x = NULL,
    y = NULL)
...
affiliationMapWorld <- ggplot() +
  geom_map(spatial_data, 
           map = "world",
           mapping = aes(fill = affiliations, geometry = geometry),
           color = "#ffffff", size = 0.25) +
  labs(fill = "Author affiliations",
       title = "Number of institutions in author affilitations by country",
    x = NULL,
...
colnames(types)<- c("SchoolType", "Number of Insitutions in US", "Percent in US", "Number of Institutes in LSE", "Percent in LSE")
view(types)
...
types %>%
  ...
  filter(name == "Percent in US" | name == "Percent in LSE") %>%
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "What types of institutes are represented in LSE?",
    subtitle = "Percent of institutions by type and control represented in LSE and US"
    )

ggsave("figures/SchoolTypes.jpg", device = "jpeg", dpi = 400)
...
ggraph(Uninet$graph, layout = "auto") +
  ...
  geom_node_label(aes(label = label,color = color), repel = TRUE, label.padding = 0.01 )+
  theme_void() + 
  theme(legend.position = 'none') +
  coord_flip() +
  labs(
...
ggplot((Aff_Freq)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "Number of papers by affiliated universities"
  )
...
# Plot the network
DCA = networkPlot(DCAMatrix, n = dim(DCAMatrix)[1], Title = "Co-Citation Network", type = "auto", size.cex = TRUE, remove.multiple=FALSE, labelsize = 10, edgesize = 5, edges.min = 5, cluster = "louvain", remove.isolates = TRUE, label.cex = TRUE)


names(DCA$nodeDegree["american association 2011-1"])<- "AAAS 2011"

ggraph(DCA$graph, layout = "auto") +
  geom_edge_link(aes(edge_width = num / 200, color = color, alpha = 0.1, edge_linetype = factor(lty)), edge_alpha = 0.1) + 
...
ggsave("figures/DocumentCocitationNetwork.jpg", device = "jpeg", dpi = 400)
