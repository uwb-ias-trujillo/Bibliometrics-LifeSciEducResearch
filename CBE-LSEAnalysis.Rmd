...
prodplot <- ggplot(data = AnnualProduction, 
  ...
  labs(
    ...
    subtitle = "Articles published per year",
    x = NULL,
    y = NULL,
  )
...
authorprod <- ggplot(df2, aes(x = rev(.data$Author), y = .data$year, group =  rev(.data$Author)
  ...
  geom_line(size = 0.7, color =  "#994E95", alpha = 0.9) +
  geom_text_repel(aes(label = .data$freq))+
  #geom_violin(data = df, aes(x =  rev(.data$Author), y = .data$year, group = rev(.data$Author)), fill =  "#994E95", color ="#994E95", adjust = 0.4, scale = "area", alpha = 0.1) +
  #geom_label()+
  geom_point(aes(size = .data$freq), color = "#FFFFFF") +
  #geom_text(aes(label = .data$freq), color = "#000000") +
  ...
  theme_default+
    theme(legend.position = "bottom",
        ...
        legend.box="horizontal",
          ...
          axis.title.y = ggtext::element_markdown(vjust = 1, angle = 0, hjust = 0),
    ...
    labs(title = "B. Author productivity", 
         subtitle = "Articles authored per year ordered by *h*",
         x = NULL, y = NULL)# +
...
ggplot(df, aes(x = .data$Author, y = .data$year, group =  .data$Author),
                               text = paste("Author: ", .data$Author,
                                     "\nYear: ", .data$year,
                                     "\nN. of Articles: ", .data$freq, 
                                     "\nTotal Citations per Year: ", round(.data$TCpY, 2)
  ...
  geom_flat_violin(fill =  "#994E95", color ="#994E95", adjust = .2, scale = "count", alpha = 0.3, width = 3)+ 
    scale_size(range = c(2, 6), 
               breaks = c(1,3,5,7)) + 
    scale_alpha(range = c(0.3, 1), 
                breaks =  c(0,20,40,60)) + 
...
ArticlesUSMap <- ggplot() +
  ...
  geom_point(data = centroids, aes(x = longitude, y = latitude, size = articles*100), color = "#333333") +
...
```{r combining maps}
ggarrange(AffiliationWorldMap, ArticlesUSMap,common.legend = TRUE, legend = "none", ncol = 1, align = c("hv"))
...
clustername <- as.data.frame(CS30[["km.res"]]$cluster)
...
TitleTerms %>% 
  ...
  arrange(value, .by_group = TRUE) %>%
  top_n(5) %>%
  ggplot(aes(Year, perc, group = name, color=cluster, fill=name, label = name))+
  geom_smooth(n = 3, se = FALSE) + 
