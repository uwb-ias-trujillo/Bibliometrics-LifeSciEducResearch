...
prodplot <- ggplot(data = AnnualProduction, 
  ...
  scale_y_continuous(limits = c(40,114)) +
  theme_default +
...
authorprod <- ggplot(df2, aes(x = .data$Author, y = .data$year,
  ...
  #geom_rect(data = df2, aes(x = .data$Author, y = 2008, width = 2, color =  "#FFFFFF", alpha = 0.9)) + 
    scale_size(range = c(2, 6), 
               breaks = c(1,3,5,7)) + 
    scale_alpha(range = c(0.3, 1), 
                breaks =  c(0,20,40,60)) + 
    scale_y_continuous(breaks = c(2010,2015,2020)) +
  ...
  theme_default+
    theme(legend.position = "bottom",
        ...
        plot.title.position = "plot", legend.margin=margin())+
            guides(fill=guide_legend(nrows = 2 ,bycol = TRUE)) + 
    labs(title = "Author productivity from 2008 - 2022", subtitle = "Author name by H-index", x = NULL, y = NULL) +
...
AffiliationWorldMap <- countries %>%
  ...
  geom_point(data = worldcentroids, aes(x = longitude, y = latitude, size = Freq *8, group = region)) +
    geom_text(data = worldcentroids, aes(x = longitude, y = latitude, label = Freq, size = Freq*2, group = region), color = "white") +
  ...
  labs(fill = "Count of article",
       title = "A. Articles by author's country affiliation",
    x = NULL,
    y = NULL
    ) 
...
ArticlesUSMap <- ggplot() +
  ...
  geom_point(data = centroids, aes(x = longitude, y = latitude, size = articles*4)) +
    geom_text(data = centroids, aes(x = longitude, y = latitude, label = articles, size = articles), color = "white") +
  coord_map("moll") +
  theme_map() +
  labs(fill = "Publications",
       title = "B. Articles by author's state affiliation",
...
```{r combining maps}
ggarrange(AffiliationWorldMap, ArticlesUSMap,common.legend = TRUE, legend = "bottom", ncol = 1, align = c("hv"))
...
SchoolTypePlot<- types %>%
  ...
  mutate(Control = case_when( 
    ...
    Type = case_when( 
    str_detect(SchoolType, "2") ~ "2-Year",
    str_detect(SchoolType, "4") ~ "4-Year"),
    Type = as.factor(Type)) %>%
  ggplot(aes(x = (name), value,fill = SchoolType)) +
    geom_col(fill = "white",
               position = "dodge",
               size = 4) +
    geom_col(aes(alpha = as.numeric(colorlevel)),
               position = "dodge",
               size = 4) +
  ...
  labs(x = NULL, y = NULL,
       title = "A. Types of US institutions represented in LSE"
       ) +
  coord_flip()+
  facet_grid(rows =vars(SchoolType), scales = "free_y", as.table = TRUE, #nrow = 4, ncol = 1, 
             #strip.position = "left", #dir = "h"
             switch = "x"
             )+
...
MSIAffiliationPlot <- temp %>% 
  filter(eligibility != "Ineligible",
    ...
    name %in% c("TCCU", "ANNH","PBI F","PPOHA","AANAPISI F", "HBGI","HBCU","HSI", "AANAPISI"),
  ...
  scale_fill_manual(values = c("#1d4f60","#6dbc90"
                               )) +
  theme_default + 
  labs(
    title = "B. US Author affiliations by Minority Serving Institution status",
    y = NULL, 
    x = NULL
  )  +
...
AuAffiliationPlot<- ggplot(Affiliation,aes(x = reorder(str_to_title(AFF), Freq), y = Freq)) +
  ...
  labs(
    x = NULL,
    y = NULL,
    title = "C. Top affiliations of authors",
  )+
...
ggarrange(SchoolTypePlot, MSIAffiliationPlot,AuAffiliationPlot, ncol = 1, common.legend = FALSE, align = c("hv"))
