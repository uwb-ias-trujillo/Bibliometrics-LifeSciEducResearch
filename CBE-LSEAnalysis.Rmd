...
# CS5 <- bibliometrix::conceptualStructure(data,
#                           field = "AB",
#                           ngrams = c(1),
#                           method = "MCA",
#                           minDegree= 5,
...
clustername <- as.data.frame(CS15[["km.res"]]$cluster)
...
TitleTerms %>% 
  ...
  arrange(value, .by_group = TRUE) %>%
  top_n(5) %>%
  ggplot(aes(Year, perc, group = name, color=cluster, group = cluster, fill=name, label = name))+
  geom_line() + 
...
TitleTerms %>% 
  ...
  arrange(value, .by_group = TRUE) %>%
  top_n(5) %>%
  ggplot(aes(Year, perc, group = name, color=name, fill=cluster, label = name))+
  geom_area() + 
  geom_text_repel(aes(label = name), data = . %>% group_by(name) %>% top_n(1, Year), 
                  size = 2, 
                  color = "black",
