...
prodplot <- ggplot(data = AnnualProduction, 
  ...
  geom_text(data = AnnualProduction %>% filter(Year %in% c(2015)), 
            aes(x = Year, y= 114, label = "Dolan"), color = "Black", hjust = 0, vjust = 0) +
  geom_text(data = AnnualProduction %>% filter(Year %in% c(2020)), 
            aes(x = Year, y= 113, label = " Schinske &\n Tanner"), color = "Black", hjust = 0, vjust = 1, nudge_y = 0) +
  geom_text_repel(data = AnnualProduction %>% filter(Year %in% c(2010, 2013, 2016)), 
                  aes(x = Year, y= Articles, label = c(Articles)), min.segment.length = 1, vjust = "middle", hjust =1, box.padding = 0.1) +
  geom_text_repel(data = AnnualProduction %>% filter(Year %in% c(2011, 2015)), 
                  aes(x = Year, y= Articles, label = c(Articles)), min.segment.length = 1, vjust = "top", hjust = "right", box.padding = 0, nudge_x = 0, nudge_y = 0) +
  geom_text(data = AnnualProduction %>% filter(Year %in% c(2022), Indexed == "Projected"), 
            aes(x = Year, y= Articles, label = "Projected\n 85"), vjust = 0, hjust = 1) +
...
AU2$Author<-fct_reorder(AU2$Author,
                        AU2$medianY,
                        #AU2$h_index,
                        .desc = FALSE)
  
levels(df2$Author) <- levels(AU2$Author)
x <- c(0.5, 1.5 * k/10)
