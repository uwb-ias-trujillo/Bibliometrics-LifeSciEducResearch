...
options(width=100)
journals<-c( "ADV PHYSIOL EDUC","AM BIOL TEACH", "BIOCHEM MOL BIOL EDU", "CBE-LIFE SCI EDUC", "J BIOL EDUC","J MICROBIOL BIOL EDU", "REV ENSINO BIOQUIM")

data <- data %>% 
  filter(J9 %in% journals, PY < 2024)
...
prodplot <- ggplot(data = AnnualProduction, 
  ...
  geom_line(stat = "identity", 
            color = "#266b6e",
            size = 1.5) +
  geom_text_repel(data = AnnualProduction %>% 
                    filter(Year %in% c(2010, 2013,2016, 2021)), 
                  aes(x = Year, y= Articles, label = c(Articles)), 
                  min.segment.length = 1, vjust = "top", 
                  hjust = "center", box.padding = 0.4) +
  geom_text_repel(data = AnnualProduction %>% 
                    filter(Year %in% c(2017, 2015, 2022)), 
                  aes(x = Year, y= Articles, label = c(Articles)), 
                  min.segment.length = 1, vjust = "bottom", 
...
Sections are modified from the functions in the bibliometrix package.
...
authorprod <- sumauthor %>%
  ...
  geom_point(fill =  "#994E95", color ="#994E95")+ 
     ...
     #scale_alpha(range = c(0.3, 1), 
                # breaks =  c(0,20,40,60)) + 
    scale_y_continuous(breaks = c(2010,2015,2020), limits = c(2010, 2023)) +
...
ggsave("figures/Prodcommon.jpg", Prodcommon,
       device = "jpeg", dpi = 400, bg = "white",
       width = 6,
       height = 8, 
       units = "in", 
...
```{r combing journal and author figure eps tiff pdf, skip = TRUE}
ggsave("figures/Prodcommon.eps", Prodcommon,
       device = "eps", dpi = 400, bg = "white",
       width = 17.57,
       height = 13.1775, 
       units = "cm", 
       limitsize = FALSE)

ggsave("figures/Prodcommon.tiff", Prodcommon,
       device = "tiff", dpi = 500, bg = "white",
       width = 17.57,
       height = 13.1775, 
       units = "cm", 
       limitsize = FALSE)

ggsave("figures/Prodcommon.pdf", Prodcommon,
       device = "pdf", dpi = 500, bg = "white",
       width = 16.57,
       height = 13.1775, 
       units = "cm", 
       limitsize = FALSE)
...
metacountries <- metaTagExtraction(data, Field = "AU_CO", sep = ";", aff.disamb = TRUE)
metaco <- unlist(strsplit(metacountries$AU_CO, ";")) %>%
  table() %>%
  as.data.frame() %>%
  rename(region = ".",coauthors = "Freq") %>%
...
#& subregion != "Chatham Island" & subregion != "12"& subregion != "13") %>%
  ...
  group_by(region, coauthors,Freq, latitude, longitude)%>%
  summarise() %>%
  group_by(region, coauthors, Freq) %>% 
  slice_head(n = 1) %>%
  filter(coauthors >0 )
...
AllAffiliationWorldMap <- allcountries %>%
  full_join(countryfreq, by = c("region" = "Tab")) %>%
  full_join(metaco) %>%
  ggplot(aes(fill = coauthors, map_id = region), color = 1) +
...
world 
ggsave("figures/MapWorld.jpg",
       device = "jpeg", dpi = 400,
       width = 4, height = 8,
       units = "in", bg = "white", limitsize = FALSE)

ggarrange(world,ArticlesUSMap,common.legend = TRUE,labels = "AUTO", 
          legend = "none", 
          #legend = "bottom",
          ncol = 1, nrow =2, heights =  c(3,1.3),#widths = c(3,1), 
          align = c("v")
)
...
```{r combining maps eps tiff pdf, skip = TRUE}
ggsave("figures/MapCombo.eps",
       device = "eps", dpi = 400,
       width = 17.57,
       height = 15.37375, 
       units = "cm",
...
SchoolTypePlot <- ggplot(SchoolTypeTable,aes(x = SchoolType, value * Sign ,fill = SchoolType
  ...
  geom_text_repel(aes(y = value, #* Sign ,
...
MSIAffiliationPlot <- temp %>% 
  ...
  geom_text_repel(aes(#y = value, #* Sign ,
    label = if_else(perc > 0.01, paste0(round(perc*100,1),"%"),"")),
    size = 3, fontface = "bold", 
    direction = "x",
    nudge_y	= -0.001,
    #point.padding	=0.5,
  ...
  coord_flip() +
  #facet_grid(rows = vars(titled), scales = "free", space = "free")+
  theme_default + 
  labs(
  ...
  theme(legend.title = element_blank(),
        ...
        axis.text.y = element_text(margin = margin(r = -0.4, l = 0.1, unit = "cm")),
...
msisummary_n %>% 
  filter(eligibility == "Current grantee", titled == TRUE) %>% 
  #select(!titled) %>%
  t()
...
ggarrange(AuAffiliationPlot, SchoolTypePlot, MSIAffiliationPlot, ncol = 1, nrow = 3,labels = "AUTO", common.legend = FALSE, align = c("v"))
...
```{r top affiliations combined eps tiff pdf, skip = TRUE}
ggsave("figures/AffiliationsCombo.eps",
       device = "eps", dpi = 400,
       width = 17.57,
       height = 15.37375, 
       units = "cm",
...
ggsave("figures/AffiliationsCombo.tiff",
       device = "tiff", dpi = 400,
       width = 17.57,
       height = 15.37375, 
       units = "cm",
...
ggsave("figures/AffiliationsCombo.pdf",
       device = "pdf", dpi = 500,
       width = 17.57,
       height = 15.37375, 
       units = "cm",
...
write.csv(file = "tables/Citations.csv",CRtable)

# Sources Table
sourcestable <-cbind(summary(factor(CR$Source))[1:10])
#sourcestable

write.csv(file = "tables/Sources.csv",sourcestable)
```
...
CS2<- bibliometrix::conceptualStructure(data, 
                          field ="ID_TM", 
                          ngrams = c(1,2,3,4),  
                          method = "MCA", 
                          minDegree= 5,
...
CS2$graph_terms+
  ...
  labs(title = "") +
  theme(legend.position = "none", axis.text = element_text(size = 14))
ggsave("figures/2ConceptualStructure.png", device = "png")
...
clustername <- as.data.frame(CS2[["km.res"]]$cluster)
...
```{r title topics scaled}
TitleTermsScaled <- TitleTerms %>% 
  filter(name %in% TopTerms$name) %>%
  group_by(name) %>%
    mutate(
        value = as.numeric(as.character(value)),
        #value = rawvalue +1,
        perc.scaled = ((perc - mean(perc)) / (sd(perc))),
...
TitleTermsScaled%>%
  #top_n(5) %>%
  ggplot(aes(Year, perc.scaled, color=as.factor(cluster), fill=name, label = name))+
  geom_line() + 
  geom_text_repel(aes(label = name), data = . %>% group_by(name) %>% top_n(1, Year), 
                  size = 2, 
                  color = "black",
...
```{r conceptual structure with ID, skip = TRUE, eval = FALSE, echo = FALSE}
IDTerms <- termExtraction(data, "ID",stemming = TRUE, keep.terms = compound) %>%
  KeywordGrowth(Tag= "ID", top = 1000) %>%
  pivot_longer(cols = !c('Year')) %>%
  group_by(Year) %>%
  mutate(sum = sum(value), perc = value / sum) %>%
  #filter(value > 1) %>%
...
TopIDTerms<- IDTerms %>% 
  filter(name %in% SharedIDTerms$name) %>%
  group_by(Year) %>%
  arrange(value, .by_group = TRUE) #%>%
  #top_n(15)
...
IDTerms %>% 
  filter(name %in% TopIDTerms$name) %>%
  left_join(clustername)%>%
  group_by(cluster) %>%
  arrange(value, .by_group = TRUE) %>%
  top_n(5) %>%
...
IDTerms %>% 
  ...
  theme_default +
  facet_wrap(~cluster)+
  labs(title = "Conceptual trends based on article keywords")+
  theme(legend.position="none")
```


```{r ID topics scaled}
IDTermsScaled <- IDTerms %>% 
  filter(name %in% TopIDTerms$name) %>%
  group_by(name) %>%
    mutate(
        value = as.numeric(as.character(value)),
...
```{r co-citation analysis}
CoCiteNet<-biblioNetwork(
  data,
...
CoCiteGraph<- networkPlot(CoCiteNet,n = 100, 
                          type = "auto", Title = "Co-Citation",labelsize=0.5, edges.min = 2, cluster = "louvain")

CoCiteGraph<-CoCiteGraph$graph
ggraph(CoCiteGraph, layout = 'auto') + 
  geom_edge_link() + 
  geom_node_point(aes()) + 
...
```{r bibliographic coupling analysis}
bibcoupNet<-biblioNetwork(
  data,
  analysis = "coupling",
  network = "references",
  n = NULL,
...
networkPlot(n = 1000, type = "kamada", Title = "Document Co-Citation",labelsize=0.5)
bibcoupNet
```


```{r institutional coupling analysis}
CoInstNet<-biblioNetwork(
  data,
  analysis = "coupling",
...
networkPlot(n = 30, type = "kamada", Title = "Collaborative institutions",labelsize=0.5) 
```
