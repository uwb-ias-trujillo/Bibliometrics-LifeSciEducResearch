...
#& subregion != "Chatham Island" & subregion != "12"& subregion != "13") %>%
  summarise(latitude = sign(lat) * exp(mean(log(abs(lat)))), longitude = sign(long) * exp(mean(log(abs(long))))) %>%
  group_by(region, Freq, latitude, longitude)%>%summarise()
...
AllAffiliationWorldMap <- countries %>%
  ...
  geom_map(map = world_map, colour = "light grey", lwd = 0.2)+
  geom_autopoint(data = All_worldcentroids %>% group_by(region, Freq), aes(x = longitude, y = latitude, size = Freq*100, group = region,),color = "#333333") +
...
ArticlesUSMap <- ggplot() +
  ...
  labs(fill = "Publications",
       title = "B. Number of Documents by state of corresponding authors",
...
SchoolTypePlot<- types %>%
  ...
  labs(x = NULL, y = NULL,
       title = "C. School types of authors compared to all U.S."
       ) +
  coord_flip()+
...
MSIAffiliationPlot <- temp %>% 
  ...
  geom_col(position = position_stack(reverse = TRUE),  fill = "#266b6e") +
  geom_text(
    aes(label =n),
...
ggarrange(AuAffiliationPlot, SchoolTypePlot, MSIAffiliationPlot, ncol = 1, common.legend = FALSE, align = c("hv"))
