...
authorProdOverTime <- function (M, k = 10, graph = TRUE) 
{
    ...
    nAU <- lengths(listAU)
    df <- data.frame(AU = trimws(unlist(listAU)), SR = rep(M$SR, 
        nAU))
...
AffiliationWorldMap <- countries %>%
  ...
  labs(fill = "Count of affiliations",
       title = "Frequency of author affilitations by country",
    x = NULL,
    y = NULL
    ) 
AffiliationWorldMap
...
affiliationMap <- ggplot() +
  ...
  labs(fill = "Author affiliations",
       title = "Number of LSE author affilitations by state",
    x = NULL,
    y = NULL
    )  +
...
ArticlesUSMap <- ggplot() +
  ...
  labs(fill = "Publications",
       title = "Number of LSE publication by state of author affiliation",
    x = NULL,
    y = NULL
    ) +
  scale_fill_carto_c(palette = "BluGrn", direction = 1) +
...
```{r combining maps}
ggarrange(AffiliationWorldMap,ArticlesUSMap, legend = "bottom", nrow = 2, ncol = 1#, common.legend = TRUE
          )
```
...
types %>%
  ...
  scale_y_continuous(minor_breaks = NULL,labels = scales::percent) +
  labs(
    x = NULL,
    y = NULL,
    title = "Percent of institutions by type represented in LSE author affiliations and US Dept. of Ed.",
    caption = "Source: CBE-LSE, 2020 US Dept. of Ed. Eligibility Matrix"
    )+
  facet_wrap(~SchoolType, nrow = 1)
...
MSIAffiliationPlot <- temp %>% 
  ...
  labs(
    ...
    subtitle  = "Percent of the 479 CBE-LSE author affilations found in 2020 US Dept. of Ed. Eligibility Matrix",
    y = NULL, 
    x = NULL
  )  +
...
MSIAffiliationPlot
ggsave("figures/MsiAffiliations.jpg", device = "jpeg", dpi = 400)
...
ggplot(Affiliation,aes(x = reorder(str_to_title(AFF), Freq), y = Freq)) +  
  ...
  labs(
    ...
    subtitle = "The frequency distribution of affiliations of all co-authors for each paper"
  )+
...
ggsave("figures/AffliationsTop10.jpg", device = "jpeg", dpi = 400)
