...
testresults <- biblioAnalysis(scientometrics, sep = ";")
testauthors=gsub(","," ",names(testresults$Authors))
test <- Hindex(scientometrics, field = "author", elements = NULL, sep = ";")
test <- Hindex(scientometrics, field = "author", elements = testauthors, sep = ";")
test

indices <- Hindex(data, field = "author", elements = authors, sep = ";", years = Inf)
#indices <- Hindex(data, field = "author", sep = ";", years = 50)$H
...
authorProdOverTime(data, k = 10, graph = TRUE)$graph + labs(x = NULL, y = NULL)
...
ggplot(authordata, aes( x = NP, y = h_index)) + 
  ...
  ggtitle("Author productivity", subtitle = "Number of publications and h-index for authors between 2010 and 2021")  #geom_smooth()
...
MSIAffiliationPlot <- temp %>% 
  ...
  theme_default + 
  labs(
    title = "Percent of author affiliations that are also Minority Serving Institution",
    caption = "Percent of 479 CBE-LSE author affilations found in 2020 US Dept. of Ed. Eligibility Matrix",
    y = NULL, 
    x = NULL
  )  +
...
```{r eligibility tables, include = FALSE}
#Eligibility tables
msiaanapisi <- msiuniv %>%
  group_by(AANAPISI) %>%
  summarize(n = n())
