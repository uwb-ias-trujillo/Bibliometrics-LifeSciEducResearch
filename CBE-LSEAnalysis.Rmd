...
ggraph(CoANet$graph, layout = "kk") +
  ...
  geom_node_label(aes(label = label ,color = factor(community), check_overlap = TRUE, repel = TRUE))+
  theme_void() + 
...
V(CoANetdeg$graph)$components[V(CoANetdeg$graph) %in% vert_ids] <- TRUE
#$graph <- delete.vertices(CoANetdeg$graph,vert_ids)
#CoANetdeg$graph <- decompose(CoANetdeg$graph,mode="weak", max.comps = 1)

ggraph(CoANetdeg$graph, layout = "nicely") +
  geom_edge_link(aes()) + 
    geom_node_point(aes(filter = components == TRUE,#V(CoANetdeg$graph) %in% vert_ids, 
                          color = factor(community), size = deg /100,))+
