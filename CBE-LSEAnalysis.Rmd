...
ggplot(data = S$AnnualProduction, mapping = aes(x = Year..., y = Articles, group = 1)) +
  geom_line(stat = "identity", color = "#F26E50", size = 1.5) + geom_line(data = data.frame(x = as.factor(c(2021,2022)), y = c(74,40 + 25 + 20)), aes(x,y), linetype = 6, stat = "identity", color = "#F26E50", size = 1.5, alpha = 0.8)+ # added projected articles.
  #theme_dark() +
...
ggplot(Affiliation,aes(x = reorder(str_to_title(AFF), Freq), y = Freq)) +  
  ...
  geom_text(
    ...
    fill = "white", label.size = 0, color="white"
  ) +
  geom_text(
    aes(label = Freq),
    hjust = 0, nudge_y = .5,
...
TitleTerms <- termExtraction(data, "TI",stemming = FALSE, keep.terms = compound) %>%
...
TitleTerms %>% 
  filter(name %in% TopTerms$name) %>%
  ggplot(aes(Year, reorder(as.factor(str_to_sentence(name)),cumsum(value)), group = name, color=name, fill=name, label = name, size = perc), alpha = 0.3)+
  ...
  geom_text(aes(label = str_to_sentence(name)), data = . %>% group_by(name) %>% top_n(-1, Year), #%>% filter(Year > 2012), 
                  ...
                  #box.padding = 0.5
                  nudge_x = 0,hjust = -0)+
  scale_x_continuous(labels = label_date(format = "%y")) +
  scale_y_discrete()+
  theme_default +
...
ggsave("figures/TitleTrends.png", device = "png")
```
