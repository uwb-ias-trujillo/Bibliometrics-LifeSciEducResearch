...
AffiliationWorldMap <- countries %>%
  ...
  geom_point(data = worldcentroids, aes(x = longitude, y = latitude, size = Freq, group = region)) +
    geom_text(data = worldcentroids, aes(x = longitude, y = latitude, label = Freq, size = Freq/8, group = region), color = "white") +
...
ArticlesUSMap <- ggplot() +
  ...
  geom_point(data = centroids, aes(x = longitude, y = latitude, size = articles)) +
    geom_text(data = centroids, aes(x = longitude, y = latitude, label = articles, size = articles/8), color = "white") +
  coord_map("moll") +
  theme_map() +
...
MSIAffiliationPlot <- temp %>% 
  filter(eligibility != "Ineligible",
    ...
    name %in% c("TCCU", "ANNH","PBI F","PPOHA","AANAPISI F", "HBGI","HBCU","HSI"),
    name %in% c("SIP","HBCU Masters","PBI A", "AANAPISI","HSI STEM")
