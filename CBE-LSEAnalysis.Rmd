...
prodplot <- ggplot(data = AnnualProduction, 
  ...
  scale_x_discrete(breaks = function(x){x[if_else(x == 2010 |x == 2015 | x == 2020, TRUE,FALSE,)]}, expand = expansion(add = c(2, 0)))+
  ...
  labs(
    ...
    subtitle = "Editors and the number of articles published per year",
    x = NULL,
    y = NULL,
  )
...
authorprod <- sumauthor %>%
  ...
  geom_point(fill =  "#994E95", color ="#994E95")+ 
     ...
     scale_alpha(range = c(0.3, 1), 
                 breaks =  c(0,20,40,60)) + 
    scale_y_continuous(breaks = c(2010,2015,2020), limits = c(2007, 2022), expand = expansion(add = c(1, 1))) +
  ...
  theme_default+
    ...
    labs(title = "B. Author productivity", 
         subtitle =expression("Articles authored per year ordered by ","h"),
         x = NULL, y = NULL) +
...
SchoolTypePlot<- types %>%
  ...
  mutate(Control = case_when( 
    ...
    Sign = case_when( 
      str_detect(SchoolType, "2") ~ -1,
      str_detect(SchoolType, "4") ~ 1),
      Type = as.factor(Type)) %>%
  ggplot(aes(x = (name), value * Sign ,fill = SchoolType)) +
    geom_col(fill = "white",
               #position = "stack",
               size = 4,
  ...
  labs(x = NULL, y = NULL,
       title = "A. School types of authors compared to all U.S."
       ) +
  coord_flip()+
  # facet_grid(cols =vars(Type), scales = "free", as.table = TRUE, #nrow = 4, ncol = 1,
  #            #strip.position = "left", 
  #            #dir = "h",
  #            switch = "x"
  #            )+
